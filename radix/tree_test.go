...
func TestNotZero(t *testing.T) {
	var i int
	if notZero(i) {
		t.Fatal("expected zero int to return false")
	}
	if !notZero(1) {
		t.Fatal("expected non-zero int to return true")
	}
	...
	if !notZero("hello") {
		t.Fatal("expected non-empty string to return true")
	}

	var p *testContext
	if notZero(p) {
		t.Fatal("expected nil pointer to return false")
	}
	if !notZero(newTestCtx()) {
		t.Fatal("expected non-nil pointer to return true")
	}
}
...
func TestLongestCommonPrefix(t *testing.T) {
	...
	for _, c := range cases {
		...
		if got != c.want {
			t.Fatalf("longestCommonPrefix(%q, %q) = %d, want %d", c.a, c.b, got, c.want)
		}
	}
}
...
func TestFindWildcard(t *testing.T) {
	// param wildcard
	w, i, valid := findWildcard("/users/:id")
	if w != ":id" || i != 7 || !valid {
		t.Fatalf("findWildcard('/users/:id') = %q, %d, %v; want ':id', 7, true", w, i, valid)
	}

	// catchAll wildcard
	w, i, valid = findWildcard("/files/*filepath")
	if w != "*filepath" || i != 7 || !valid {
		t.Fatalf("findWildcard('/files/*filepath') = %q, %d, %v; want '*filepath', 7, true", w, i, valid)
	}

	// param with trailing segment
	w, i, valid = findWildcard("/users/:id/posts")
	if w != ":id" || i != 7 || !valid {
		t.Fatalf("findWildcard('/users/:id/posts') = %q, %d, %v; want ':id', 7, true", w, i, valid)
	}

	// no wildcard
	w, i, valid = findWildcard("/users")
	if w != "" || i != -1 || valid {
		t.Fatalf("findWildcard('/users') = %q, %d, %v; want '', -1, false", w, i, valid)
	}

	// invalid: two wildcards in one segment
	_, _, valid = findWildcard("/:id:name")
	if valid {
		t.Fatal("findWildcard('/:id:name') should be invalid")
	}
}
...
func TestShiftNRuneBytes(t *testing.T) {
	...
	for _, c := range cases {
		got := shiftNRuneBytes(rb, c.n)
		if got != c.want {
			t.Fatalf("shiftNRuneBytes(%v, %d) = %v, want %v", rb, c.n, got, c.want)
		}
	}
}
